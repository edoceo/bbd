...
foreach ($event_list as $e) {
	...
	case 'PRESENTATION/SharePresentationEvent':
		$cur_p = $e['presentationName'];
		$k = sprintf('%s/slide-%d', $cur_p, 1);
		if (empty($media_list[$k])) {
			$media_list[$k] = array(
				'time_s' => floor($e['time'] / 1000),
				'time_ms' => $e['time'],
	...
	case 'PRESENTATION/GotoSlideEvent':
		...
		$k = sprintf('%s/slide-%d', $cur_p, intval($e['slide']) + 1 );
		if (empty($media_list[$k])) {
			$media_list[$k] = array(
				'time_s' => floor($e['time'] / 1000),
				'time_ms' => $e['time'],
	...
	case 'PRESENTATION/GenerateSlideEvent':
		// print_r($e);
		// exit;
		$p = $e['presentationName'];
		$k = sprintf('%s/slide-%d', $p, $e['pagesCompleted']);
		if (empty($media_list[$k])) {
			$media_list[$k] = array(
				'time_s' => floor($e['time'] / 1000),
				'time_ms' => $e['time'],
